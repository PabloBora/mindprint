...
(() => {
  ...
  function viewProspectos() {
    ...
    h += '<form class="addform tareas" data-act="add-prospecto">'
      ...
      + `<select class="in" id="prospecto-caso" aria-label="Oportunidad"><option value="">Oportunidad por definir</option>${S.estado.ideas.slice().sort((a, b) => String(a.titulo).localeCompare(String(b.titulo))).map((i) => `<option value="${esc(i.id)}">${esc(i.titulo)}</option>`).join('')}</select>`
