...
test('app.js parsea con node --check y solo habla con el mismo origen', () => {
  ...
  for (const ruta of ['/api/estado', '/api/eventos', '/api/ideas/', '/api/tareas/', '/api/iteracion', '/api/mensajes', '/entrar/', '/salir']) assert.ok(js.includes(ruta), `usa ${ruta}`);
  for (const tab of ['hoy', 'tareas', 'ideas', 'chat', 'iter', 'actividad']) assert.ok(js.includes(`['${tab}', `), `pestaña ${tab}`);
  for (const fn of ['function viewHoy', 'function viewTareas', 'function viewChat', 'function seccionComentarios', 'function renderDlgTarea', 'function viewIdeas', 'function viewIter', 'function viewActividad']) assert.ok(js.includes(fn), fn);
});
